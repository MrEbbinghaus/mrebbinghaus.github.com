...
In this tutorial, you will teach the basics ideas of Fulcro. 
We will show you how to write basic components and how to manage state in a normalized graph database.
...
(ns fulcro.tutorial
  (:require 
    [com.fulcrologic.dom :as dom :refer [div p h2 ul li]]))
...
Now you can feed data to your function.
...
What do you do, when you want to change the profile picture, for example? 
You will have to remember every place where you put the link! Terrifing!
...
But you can avoid this mess, by preventing that duplication.
...
This is where Fulcro comes to the resuce. 
Fulcro is first and foremost only about solving this problem.
How to organise data so that they can be modified easily, but at the same time feed it to my render functions? 
...
All well and good.. But how does the data actually get to this component? 
Let's simplify.

On the one hand, we had our data with the duplications. There where no idents, just nesting. That kind of format ist called a tree. 
...
;; Note: The functions in Fulcro are not called `normalize` and `denormalize`, but the idea ist the same.
...
Comming back to our complete code from before, we can replace the `person` function with our component.
...
;; We need an actual function to call for rendering. The prefix `ui-` is ideomatic.
...
But the real _root_ of our application is still the `profile-page` function. That's the function that get's the _one tree_ to render. So this is the point, where we need our _one query_ to start.
...
#### Where is the `:ident`?
Not every component needs an ident. In the case of the _root_-component it wouldn't even make sense. It would be like: `(get-in data [])`